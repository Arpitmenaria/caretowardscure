...
(function() {
	...
	function initAppointmentModal() {
		...
		dateInput?.addEventListener('change', function() {
			...
			// Filter time slots - disable past times for today
			...
			options?.forEach(option => {
				...
				if (selectedDate.getTime() === today.getTime()) {
					...
					if (slotTime < today) {
						...
						option.textContent = option.textContent + ' (Passed)';
					}
				}
			});

			// Clear invalid selection if it's disabled
		...
		// Update available slots when date changes (trigger filtering)
		dateInput?.addEventListener('change', function() {
			if (timeInput) {
	...
	/**
	 * Set default date to today and select first available time slot
	 */
	function setDefaultDateTime() {
		...
		if (timeInput) {
			...
			// Find first available time slot after current time
			const options = timeInput.querySelectorAll('option[value]');
			let firstAvailableSlot = '';

			options.forEach(option => {
				if (!firstAvailableSlot && !option.disabled) {
					firstAvailableSlot = option.value;
				}
			});

			timeInput.value = firstAvailableSlot;
		}
	}
